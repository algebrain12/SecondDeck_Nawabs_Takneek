...
public class CombinedSceneScript : MonoBehaviour
{
    #if UNITY_ANDROID
    ...
    [SerializeField] private float flickAccelThreshold = 1.5f;
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogWarning("[CombinedScene] Gyroscope not supported on this device.");
    ...
    void Start()
    {
        ...
        {
            Debug.LogWarning("[CombinedScene] selfData.IPAddress was empty; falling back to a fresh IP query. " +
        ...
        // NEW: look up and display the passive/active description for
        // whichever ability was rolled for this player. Same local static
        // lookup as Phone2Script - no network changes required.
    ...
    private void HandleConnected()
    {
        ...
        if (AbilityButton != null && AbilityCooldownLeft <= 0f)
            AbilityButton.interactable = true;
    }

    private void HandleConnectionFailed(string errorMessage)
    {
        Debug.LogError($"[CombinedScene] Failed to connect to host: {errorMessage}");
    ...
    public void FireLeftB()
    {
        ...
        {
            Debug.LogWarning("[CombinedScene] FireLeftB ignored: not connected to host.");
            return;
        }
    ...
    public void FireRightB()
    {
        if (!tcpHandler.IsConnected)
        {
            Debug.LogWarning("[CombinedScene] FireRightB ignored: not connected to host.");
            return;
        }
    ...
    public void PressedAbility()
    {
        if (!tcpHandler.IsConnected)
        {
            Debug.LogWarning("[CombinedScene] PressedAbility ignored: not connected to host.");
            return;
        }
    ...
    private void DetectFlick()
    {
        ...
        {
            Debug.Log($"[CombinedScene] Left flick detected! Accel: {sidewaysAccel:F2}g");
            FireLeftB();
            lastFlickTime = Time.time;
        }
        else if (sidewaysAccel < -flickAccelThreshold)
        {
            Debug.Log($"[CombinedScene] Right flick detected! Accel: {sidewaysAccel:F2}g");
            FireRightB();
            lastFlickTime = Time.time;
        }
    }
